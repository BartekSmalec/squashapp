...
export class UserService {
  ...
  private GET_USER_BY_USERNAME = `${this.BASE_URL}users/getUserByUserName`;
  ...
  getUserByUserName(userName: string): Observable<User> {
    return this.http.get<User>(this.GET_USER_BY_USERNAME + "/" + userName);
  }

  login(username: String, password: String) {
    const headers = new HttpHeaders({ "Content-Type": "application/json" });
    return this.http.post<String>(this.LOGIN_URL, { headers: headers });
  }
}
